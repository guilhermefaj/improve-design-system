...
import { Button, Combobox, DataGrid, NumberInput, SegmentedControl, Sidebar, Stepper, type DataGridColumn } from '../components';
...
describe('SaaS component layer', () => {
  ...
  it('changes segmented and numeric values through accessible controls', () => {
    ...
    render(<><SegmentedControl label="Período" value="month" onValueChange={onSegment} items={[{ value: 'week', label: 'Semana' }, { value: 'month', label: 'Mês' }]} /><NumberInput label="Licenças" defaultValue={2} onValueChange={onNumber} /></>);
  ...
  it('filters and selects an option in the combobox', () => {
    ...
    render(<Combobox label="Plano" options={[{ value: 'growth', label: 'Growth' }, { value: 'enterprise', label: 'Enterprise' }]} onValueChange={onValueChange} />);
  ...
  it('supports keyboard navigation in the combobox', () => {
    const onValueChange = vi.fn();
    render(<Combobox label="Plano" options={[{ value: 'starter', label: 'Starter' }, { value: 'growth', label: 'Growth' }, { value: 'enterprise', label: 'Enterprise' }]} onValueChange={onValueChange} />);
    const input = screen.getByRole('combobox', { name: 'Plano' });
    fireEvent.focus(input);
  ...
  it('announces step progress and active sidebar navigation', () => {
    render(<><Stepper active={1} items={[{ id: 'a', label: 'Empresa' }, { id: 'b', label: 'Plano' }]} /><Sidebar groups={[{ items: [{ label: 'Início', href: '#', active: true }] }]} /></>);
  ...
  it('sorts and selects rows in the data grid', () => {
    ...
    const rows: Row[] = [{ id: 'b', name: 'Beta' }, { id: 'a', name: 'Alpha' }];
    const columns: DataGridColumn<Row>[] = [{ id: 'name', header: 'Nome', cell: (row) => row.name, sortValue: (row) => row.name }];
    const onSelectionChange = vi.fn();
    render(<DataGrid rows={rows} columns={columns} getRowId={(row) => row.id} caption="Clientes" selectable selectedIds={[]} onSelectionChange={onSelectionChange} />);
