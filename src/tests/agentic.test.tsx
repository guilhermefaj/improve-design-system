...
describe('agentic interaction contracts', () => {
  it('announces execution and streaming state without exposing animation as content', () => {
    render(<><AgentStatus status="running" detail="Consultando dados" /><StreamingMessage isStreaming>Resposta parcial</StreamingMessage></>);
  ...
  it('shows tool purpose, attempt, duration, recoverable error and retry', () => {
    const onRetry = vi.fn();
    render(<ToolCallCard name="CRM" purpose="Atualizar oportunidade" status="failed" attempt={2} durationMs={840} error="Tempo esgotado" onRetry={onRetry} />);
  ...
  it('requires explicit human approval and supports edit and rejection', () => {
    ...
    render(<ApprovalCard title="Publicar relatório" action="Enviar documento" system="Google Drive" destination="Pasta do cliente" dataScopes={['Relatório executivo']} consequence="O cliente receberá a nova versão." risk="high" reversible={false} onApprove={onApprove} onEdit={onEdit} onReject={onReject} />);
  ...
  it('represents resumable plans, permissions and handoffs with accessible structure', () => {
    render(<>
      <PlanSteps steps={[{ id: '1', label: 'Pesquisar', status: 'completed' }, { id: '2', label: 'Revisar', status: 'in_progress' }]} />
    ...
    expect(screen.getByRole('region', { name: 'Transferência de Pesquisador para Revisor' })).toHaveTextContent('Validar as fontes');
  });

  it('keeps provenance and generated boundaries inspectable', () => {
    const events = [{ id: 'e1', runId: 'r1', type: 'search', status: 'succeeded' as const, timestamp: '2026-08-12T12:00:00Z', label: 'Pesquisa concluída' }];
    render(<>
      <CitationList items={[{ id: 'c1', title: 'Fonte primária', source: 'Improve', href: 'https://improve.business', verified: true }]} />
